...
- (IBAction)feedBackButton:(UIButton *)sender {
    ...
    XMIdeaViewController *userResponseViewController = [[XMIdeaViewController alloc] init];
...
- (IBAction)buttonClickedToChooseUserMood:(UIButton *)sender {
    ...
    if (sender.tag == 101) {
        ...
        [self ShowSingleBarWithTitle:barTitle content:self.hopeArray inputType:@"3"];
        
    }else if (sender.tag == 104) {
        
        NSString *barTitle = @"选择最不爽的更新!";
        [self ShowSingleBarWithTitle:barTitle content:self.nowArray inputType:@"4"];
...
- (void)ShowSingleBarWithTitle:(NSString *)title content:(NSArray *)content inputType:(NSString *)input
{
    ...
    //    float height = self.view.frame.size.height * 0.7;
        float height = 350;
