...
- (void)setupPieContentViewWith:(NSArray *)array
{
    ...
    if (self) {
        ...
        for (int index = 0; index < array.count; ++index) {
            ...
            NSLog(@"------------%@:%d", dict[@"appupdatelog"],count);
        ...
        CGFloat pieChartY = CGRectGetMaxY(pieChart.frame) + 20;
        
        // 信息
        [self showPieChartInfoWithArray:array pieChartY:pieChartY];
    }

}

- (void)showPieChartInfoWithArray:(NSArray *)array pieChartY:(CGFloat)y
{
    // 计算占有率
    float allCount = 0;
    for (int index = 0; index < array.count; ++index) {
...
//- (void)SetupPieContentView
//{
//    if (self)
//    {
//        int height = [self bounds].size.width/5*4.; // 220;
//        int width = [self bounds].size.width; //320;
//        PCPieChart *pieChart = [[PCPieChart alloc] initWithFrame:CGRectMake(([self bounds].size.width-width)/2,([self bounds].size.height-height)/4,width,height)];
//        [pieChart setBackgroundColor:[UIColor clearColor]];
...
//        [self addSubview:pieChart];
//        
//        NSDictionary *sampleInfo = @{@"data" : @[@{@"title" : @"", @"value" : @20},
//                                                 @{@"title" : @"", @"value" : @20},
//                                                 @{@"title" : @"", @"value" : @20},
//                                                 @{@"title" : @"", @"value" : @20},
//                                                 @{@"title" : @"", @"value" : @20}]};
