...
//    [headView setImageWithURL:[NSURL URLWithString:_repairman.photo]];
    ...
    if ([_devote.photo isEqualToString:@""]) {
        picture.image=[UIImage imageNamed:@"picture01"];
    }{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 0) {
        //姓名
        name=[[QHLabel alloc]initWithFrame:CGRectMake(picture.right+15, 15, 1000, 25)];
        ...
        //修神修龄
        year=[[QHLabel alloc]initWithFrame:CGRectMake(name.left, name.bottom, 100, 20)];
        ...
        //技能
        master=[[QHLabel alloc]initWithFrame:CGRectMake(15, 15, 1000, 20)];
        master.text=@"修神精通";
        ...
        //描述
        masterLabel=[[QHLabel alloc]initWithFrame:CGRectMake(master.right+10, master.top, kDeviceWidth-master.width-40, 1000)];
        ...
        //认证
        skill=[[QHLabel alloc]initWithFrame:CGRectMake(15, 15, master.width, 20)];
        skill.text=@"技术认证";
        ...
        if ([_devote.tach isKindOfClass:[NSArray class]] && _devote.tach.count) {
            ...
            for (int i=0; i<_devote.tach.count; i++) {
                ...
                [skillView setImageWithURL:[NSURL URLWithString:url]];
            ...
            QHLabel *label = [[QHLabel alloc] initWithFrame:CGRectMake(skill.right+10, skill.top, 100, skill.height)];
            label.text = @"无";
...
-(void)approveAction{
    ...
    UIAlertView  *alert1=[[UIAlertView alloc]initWithTitle:@"修神100%实名认证\n岗前严格背景调查" message:nil delegate:nil cancelButtonTitle:@"知道了" otherButtonTitles:nil, nil];
    [alert1 show];
...
- (void)actionTap:(UITapGestureRecognizer *)sender
{
    ...
    if ([[imageView superview] isKindOfClass:[UITableView class]]) {
        ...
        frame_first=CGRectMake(cell.frame.origin.x+imageView.frame.origin.x, cell.frame.origin.y+80-_tableView.contentOffset.y, imageView.frame.size.width, imageView.frame.size.height);
    }
