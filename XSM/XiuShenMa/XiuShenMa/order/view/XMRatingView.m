...
- (void)layoutSubviews
{
    ...
    for (int index = 0; index < 5; index++) {
        ...
        if (self.style == kSmallStyle) {
            ...
            grayStar.frame = CGRectMake(0+width, 5, kSmallWidth, kSmallHeight);
            width += kSmallWidth;
        }else {
            yellowStar.frame = CGRectMake(0+width, 0, kNormalWidth, kNormalHeight);
            grayStar.frame = CGRectMake(0+width, 5, kNormalWidth, kNormalHeight);
            width += kNormalWidth;
        }
    }
    ...
    float height = 0;
    if (self.style == kSmallStyle) {
        _baseView.frame = CGRectMake(0, 5, baseViewWidth, kSmallHeight);
        _ratingLabel.font = [UIFont boldSystemFontOfSize:kSmallFontSize];
        height = kSmallHeight;
    }else {
        _baseView.frame = CGRectMake(0, 5, baseViewWidth, kNormalHeight);
...
//    [_ratingLabel sizeToFit];
    ...
    // 设置视图的frame
    self.frame = CGRectMake(self.frame.origin.x-width, self.frame.origin.y, width, height);
